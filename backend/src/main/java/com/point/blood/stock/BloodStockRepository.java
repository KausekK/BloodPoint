...
//TODO
    ...
    @Query("""
        SELECT s
        FROM BloodPointBloodTyp s
        WHERE s.donationPoint.id = :pointId
          AND s.bloodType.id     = :bloodTypeId
    """)
    Optional<BloodPointBloodTyp> findForUpdate(@Param("pointId") Long pointId,
